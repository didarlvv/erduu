...
export async function login(username: string, password: string): Promise<LoginResponse> {
  try {
    ...
    StorageService.setItem("permissions", JSON.stringify(data.payload.roles.flatMap((role) => role.permissions)))
    StorageService.setItem("additionalPermissions", JSON.stringify(data.payload.additional_permissions))
...
export async function validateToken(token: string): Promise<boolean> {
  try {
    const response = await axios.post<ValidateTokenResponse>(`${API_URL}/authentication/validate/token`, {
      token,
    })
...
export async function refreshToken(refreshToken: string): Promise<string | null> {
  try {
    const response = await axios.post<RefreshTokenResponse>(`${API_URL}/authentication/generate/token`, {
      token: refreshToken,
    })
...
export async function validateTokenOnce(): Promise<boolean> {
  ...
  const validationTimestamp = StorageService.getItem("tokenValidationTimestamp")
...
export function getPermissions() {
  ...
  const additionalPermissionsString = StorageService.getItem("additionalPermissions")

  return [...(JSON.parse(permissionsString ?? "[]") ?? []), ...(JSON.parse(additionalPermissionsString ?? "[]") ?? [])]
}
