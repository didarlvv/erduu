...
export function useSocket() {
  ...
  const emitGetRoomMembers = useCallback(
    (reference: string, mailId: number) => {
      if (socket && isConnected) {
        console.log(`Socket: Emitting getRoomMembers - reference: ${reference}, mailId: ${mailId}`)
  ...
  const emitNewMessageToRoom = useCallback(
    (data: { receiver_id: number; payload: string; reference: string; mail_id: number }) => {
      if (socket && isConnected) {
        console.log(`Socket: Emitting newMessageToRoom - ${JSON.stringify(data)}`)
    ...
    (callback: (members: number[]) => void) => {
      if (socket) {
        ...
        socket.on("roomMembers", (members) => {
          console.log(`Socket: Received roomMembers event - ${JSON.stringify(members)}`)
          callback(members)
        })
      }
      return () => {
    ...
    (callback: (user: any) => void) => {
      if (socket) {
        ...
        socket.on("userJoined", (user) => {
          console.log(`Socket: Received userJoined event - ${JSON.stringify(user)}`)
          callback(user)
        })
      }
      return () => {
    ...
    (callback: (user: any) => void) => {
      if (socket) {
        ...
        socket.on("userLeft", (user) => {
          console.log(`Socket: Received userLeft event - ${JSON.stringify(user)}`)
          callback(user)
        })
      }
      return () => {
  ...
  const emitGetClientChat = useCallback(
    (clientId: number, lastMessageId?: number) => {
      if (socket && isConnected) {
        ...
        console.log(`Socket: Emitting getClientChat - ${JSON.stringify(payload)}`)
    ...
    (callback: (messages: any[]) => void) => {
      if (socket) {
        ...
        socket.on("newMessageToChat", (messages) => {
          console.log(`Socket: Received newMessageToChat event - ${JSON.stringify(messages)}`)
          callback(messages)
        })
      }
      return () => {
    ...
    (callback: (messages: any[]) => void) => {
      if (socket) {
        ...
        socket.on("createdMessage", (messages) => {
          console.log(`Socket: Received createdMessage event - ${JSON.stringify(messages)}`)
          callback(messages)
        })
      }
      return () => {
    ...
    (callback: (history: any[]) => void) => {
      if (socket) {
        ...
        socket.on("clientChats", (history) => {
          console.log(`Socket: Received clientChats event - ${JSON.stringify(history)}`)
          callback(history)
        })
      }
      return () => {
    ...
    (callback: (onlineUsers: any[]) => void) => {
      if (socket) {
        ...
        socket.on("onlineUsers", (onlineUsers) => {
          console.log(`Socket: Received onlineUsers event - ${JSON.stringify(onlineUsers)}`)
